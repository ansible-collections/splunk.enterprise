...
$spec = @{
    options = @{
        ...
        temp_dir = @{ type = "str"; required = $false; default = "C:\temp" }
        ...
        service_account_type = @{ type = "str"; required = $false; default = "local_system"; choices = @("local_system", "virtual_service_account", "domain_user") }
...
# Ensure directory exists (create it if missing) and return whether it was created.
...
function Get-SplunkForwarderRegistryInstall {
    ...
    foreach ($path in $paths) {
        ...
        foreach ($item in $items) {
            ...
            if ($item.DisplayName -like "*Splunk Universal Forwarder*" -or $item.DisplayName -like "*Universal Forwarder*" -or $item.DisplayName -like "*UniversalForwarder*") {
...
function Test-FileChecksum {
    ...
    $actualHash = (Get-FileHash -Path $filePath -Algorithm SHA512).Hash.ToUpper()
...
function Get-DesiredSplunkConfigPlan {
    param(
        ...
        [string]$splunkExe,
        [Parameter(Mandatory = $false)]
        $desiredDeploymentServer,  # No type constraint - accept null, string, anything
        [Parameter(Mandatory = $false)]
    ...
    $currentForwardServers = Get-CurrentForwardServers -splunkExe $splunkExe
    # Check if desiredForwardServers is a list (even if empty)
    if ($null -ne $desiredForwardServers -and $desiredForwardServers -is [System.Collections.IList]) {
    ...
    if ($state -eq "present") {
        ...
        elseif ($installedVersion) {
            ...
            elseif ($versionComparison -gt 0) {
                $module.FailJson("Downgrade is not supported. Installed version ($installedVersion) is higher than requested version ($version). Please uninstall the current version and then install the desired version.")
            }
...
"@
        ...
        if ($wouldWriteSeed -and (-not $module.CheckMode)) {
            ...
            Ensure-Directory -path (Split-Path -Parent $seedFile) | Out-Null
            Set-Content -Path $seedFile -Value $desiredSeed -Encoding Ascii
        ...
        if ($module.CheckMode) {
            $module.Result.changed = $wouldInstall -or $wouldUpgrade -or $wouldWriteSeed -or $wouldSetDeployPoll -or ($wouldAddForwardServers.Count -gt 0) -or ($wouldRemoveForwardServers.Count -gt 0)
            $module.ExitJson()
        }
