...
description:
  ...
  - Returns installation state, version, version hash, CPU architecture, forward servers, and deployment server configuration.
...
def get_installed_version_hash(module: AnsibleModule) -> str | None:
    ...
    if not is_splunk_installed(module):
        return None
    try:
        rc, out, err = module.run_command(['rpm', '-q', '--queryformat', '%{RELEASE}', 'splunkforwarder'])
        if rc == 0 and out:
            return out.strip()
        return None
    except Exception:
...
def get_installed_cpu_arch(module: AnsibleModule) -> str | None:
    """Get the CPU architecture of the installed Splunk package from RPM."""
    if not is_splunk_installed(module):
        return None
    try:
        rc, out, err = module.run_command(['rpm', '-q', '--queryformat', '%{ARCH}', 'splunkforwarder'])
        if rc == 0 and out:
            return out.strip()
        return None
    except Exception:
...
def get_forward_servers(module: AnsibleModule, splunk_home: str, username: str, password: str) -> list:
