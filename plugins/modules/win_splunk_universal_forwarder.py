...
options:
  ...
  purge:
    description:
      ...
      - This will remove both the installation directory and the temp directory (default C(C:\temp)) with all downloaded files.
  ...
  release_id:
    description:
      ...
      - This is the suffix between the version and platform in the MSI filename: C(splunkforwarder-<version>-<hash>-windows-x64.msi).
    type: str
  temp_dir:
    description:
  ...
  install_dir:
    description:
      ...
      - "IMPORTANT: This parameter cannot be changed after installation. The install directory is set during initial installation and cannot be moved via MSI upgrade."
      - "If you installed Splunk to a non-default directory, you MUST provide the same C(install_dir) value for all subsequent runs (upgrades, configuration changes, etc.)."
  ...
  service_account_type:
    description:
      ...
      - C(virtual_service_account) uses a virtual service account (default MSI behavior without USE_LOCAL_SYSTEM), This option is recommended for production environments.
      - When using C(virtual_service_account), there might be inconsistencies while installing and failures might occur, recommandation is to run with retry logic.
